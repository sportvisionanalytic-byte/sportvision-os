-- v152 : l'opérateur peut envoyer sa livraison à la Production (11/09/2026).
--
-- Suite de v151, trouvé en répétant l'après-match de la première mission réelle : l'écran de
-- sauvegarde propose « 📤 Envoyer à Production » (montage en cours → prête pour validation), mais
-- protect_prestation_operational_fields s'arrêtait à « médias complets » pour le collaborateur.
-- Le bouton n'apparaissait qu'en « montage en cours », statut que l'opérateur ne pouvait pas
-- atteindre, et la base refusait l'envoi. On ouvre ses trois pas de post-production ; valider et
-- livrer au club restent à la Production.
...
begin
  ...
  if new.statut is distinct from old.statut then
    is_valid_transition := (
      ...
      -- v152 : sa post-production, puis « Envoyer à Production ». La validation (prêt_validation →
      -- à_valider_client / livrée) reste à la Production.
