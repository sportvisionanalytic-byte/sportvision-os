-- ═══════════════════════════════════════════════════════════════════════════════
-- LOT 1d — Chiffrer une offre qui n'a pas de produit de catalogue
-- ═══════════════════════════════════════════════════════════════════════════════
--
-- Dernier verrou de la chaîne, et le plus discret : media_gallery_quote choisissait le modèle
-- de prix en regardant si l'offre avait un PRODUIT, pas si elle existait.
--
--     if o.product_id is not null then   -- modèle « formule »
--
-- Depuis la v31 une offre peut se suffire à elle-même. Ces offres-là retombaient donc sur le
-- modèle historique par photo, qui cherche une politique tarifaire par club — introuvable pour
-- une galerie autonome. Résultat : le devis ne renvoyait aucune ligne. Pas une erreur, RIEN,
-- ce qui se serait traduit à l'écran par un bouton d'achat qui ne fait rien.
--
-- La bascule se fait désormais sur l'existence d'une offre. Aucun prix ne change pour les
-- galeries adossées à un club : elles ont un produit, elles empruntent le même chemin qu'avant.
