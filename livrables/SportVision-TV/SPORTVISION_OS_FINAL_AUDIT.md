...
Fouka a délégué la décision sur les points 3 à 6 ; ils ont été tranchés et exécutés le matin même (voir détail après la liste). Seuls 1 et 7 restent réellement hors de portée sans son accès personnel.

1. **Non traité — Token GitHub personnel exposé en clair dans `git remote -v`** — signalé depuis le 06/08, toujours non traité. Nécessite le compte GitHub de Fouka (aucune clé SSH ni session `gh` valide en local pour basculer sur une alternative sans casser le push). À révoquer en priorité (GitHub → Settings → Developer settings → Personal access tokens), puis reconfigurer le remote sans token dans l'URL.
...
7. **Non traité — Stripe live jamais testé avec un vrai cycle de paiement complet** — nécessite une carte réelle ou des clés de test dédiées que je n'ai pas.
