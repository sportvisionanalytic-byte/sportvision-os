-- L'etape « Calendrier & evenements » de l'onboarding ne comptait que club_calendar_events.
--
-- Constate le 09/09/2026 sur SF Villemomble : 71 matchs importes depuis le planning du club, et
-- l'etape restait rouge. La carte s'intitule pourtant « 4. Calendrier & evenements », se decrit
-- comme « Matchs, tournois, stages, portes ouvertes » et porte un bouton « Importer le calendrier
-- des matchs ». Un club qui fait exactement ce que l'ecran lui demande ne voyait rien avancer.
--
-- Les matchs ne vivent pas dans club_calendar_events (evenements ponctuels saisis a la main) mais
-- dans club_matches, alimentee par l'import de calendrier. L'etape doit donc regarder les deux :
-- c'est la seule ligne qui change ci-dessous, le reste de la fonction est repris tel quel depuis
-- la base pour ne rien perdre en route.
--
-- Aucun droit n'est modifie : le controle d'acces en tete de fonction est inchange.
...
begin
  ...
  -- Un calendrier rempli, c'est des matchs OU des evenements. Les matchs arrivent par l'import
  -- que cette etape propose elle-meme ; les ignorer rendait l'etape impossible a valider par ce
  -- chemin-la (09/09/2026).
  select (
...
comment on function public.club_onboarding_completion(uuid) is
  'Progression de l''onboarding Communication d''un club, calculée EN DIRECT depuis les vraies tables (clubs/club_members/club_teams/club_team_training_slots/club_calendar_events/club_matches/club_sponsors/club_social_accounts) — jamais un pourcentage stocké séparément qui pourrait désynchroniser de la réalité. Appelable par le club (is_club_member) ou le staff uniquement (vérifié en code depuis v50, cf. faille QA du 02/09), SECURITY DEFINER pour lire uniformément malgré les RLS de chaque table sous-jacente. Depuis v51, l''étape « Calendrier » compte aussi les matchs importés (club_matches) et pas seulement les événements saisis à la main.';
