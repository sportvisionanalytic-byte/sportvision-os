-- v145 : un seul calcul du nombre de présences d'un club (11/09/2026).
--
-- Suite de v142, trouvé en balayant Club+ : la case « Cette semaine » du tableau de bord du CM
-- affichait 8 présences SportVision à Villemomble pour 2 missions. Elle comptait les événements
-- couverts du calendrier, pas les missions. Et la carte du mois, elle, oubliait les entraînements
-- couverts (seuls matchs et événements du calendrier), quand la rentabilité les comptait.
-- Trois endroits, trois règles.
--
-- nb_presences_club est désormais la seule règle : une présence = une mission (les lignes d'une
-- même mission comptent une fois), matchs, événements et entraînements du club, hors annulées.
-- La carte du mois, la case de la semaine et la rentabilité l'appellent toutes.
-- Réservée aux fonctions de la base : aucun rôle ne peut l'appeler directement (elle ne vérifie
-- pas qui demande, ce sont les fonctions appelantes qui le font).
-- Test : tests/presences-comptees-par-mission.test.sql

create or replace function public.nb_presences_club(p_club_id uuid, p_du date, p_au date)
returns integer
language sql stable security definer
set search_path = public, pg_temp
as $$
  select count(distinct coalesce(pr.created_prestation_id, pr.id))::int
    from planned_presences pr
   where coalesce(pr.statut, 'prevu') <> 'annule'
     and pr.date_presence between p_du and p_au
...
$$;
revoke execute on function public.nb_presences_club(uuid, date, date) from public, anon, authenticated;
...
begin
  ...
  v_presences := nb_presences_club(p_club_id, v_debut, v_fin);
