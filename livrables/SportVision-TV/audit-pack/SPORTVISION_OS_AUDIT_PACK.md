...
**Volumétrie réelle au moment de l'audit** (comptage agrégé, aucune donnée listée) : base en phase de rodage, pas encore en pleine exploitation commerciale — `notifications` (176 lignes) est la seule table avec un volume notable ; `clients` (18), `catalogue_offres` (13), `profiles` (11, l'effectif interne SportVision), `club_members` (4), `clubs`/`organizations` (2 chacune), `prestations` (2), `devis`/`group_fundings`/`player_profiles`/`prestations_equipe`/`team_memberships`/`club_teams` (1 chacune) ; `club_bookings`, `contrats`, `factures`, `connect_clubplus_signup_requests`, `client_organisation` : **0 ligne**.
