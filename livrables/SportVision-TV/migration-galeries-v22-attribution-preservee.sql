-- Migration : ne plus detruire l'attribution des ventes en modifiant un lien
-- À exécuter APRÈS migration-galeries-v21-analytics.sql.
--
-- ── Le défaut, trouvé en écrivant les tests d'analytics ──
-- media_link_save supprimait TOUTES les offres d'un lien puis les réinsérait. Comme
-- media_orders.offer_id est en ON DELETE SET NULL, la moindre modification de tarif effaçait
-- l'attribution de toutes les ventes passées de ce lien.
--
-- Personne ne l'aurait vu : la commande garde son montant, le chiffre d'affaires global reste
-- juste, seule la ventilation PAR OFFRE se vide. On aurait constaté un jour que « Offre
-- supprimée » représentait tout le chiffre d'affaires, sans comprendre pourquoi — et sans pouvoir
-- reconstituer la donnée, puisqu'elle serait perdue.
--
-- ── La correction ──
-- Réconcilier au lieu de remplacer : mettre à jour ce qui existe, insérer ce qui est nouveau,
-- désactiver ce qui a déjà vendu, ne supprimer que ce qui n'a jamais rien vendu.
--
-- La définition est reprise TELLE QU'ELLE EST DÉPLOYÉE : la réécrire de mémoire risquerait de
-- perdre les validations déjà en place.

begin;

drop function if exists media_link_save(uuid, jsonb, uuid, text, text, timestamptz, boolean, integer, boolean);
