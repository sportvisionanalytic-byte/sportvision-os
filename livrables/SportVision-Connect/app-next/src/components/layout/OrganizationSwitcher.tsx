...
export function OrganizationSwitcher() {
  ...
  // Les écussons des clubs proposés, en une lecture (LogoClub.tsx).
  ...
  return (
    <div ref={containerRef} className="relative px-3.5">
      ...
      >
        <PastilleClub nom={ctx.organization.name} logo={ctx.organization.logoUrl ?? logos[ctx.organization.id]} className="h-[30px] w-[30px] rounded-[9px] text-[11px]" degrade="from-brand-blue-electric to-brand-cyan" />
...
function SpaceRow({ space, isActive, onSelect, logo }: { space: Space; isActive: boolean; onSelect: () => void; logo?: string }) {
  ...
  return (
    ...
    >
      <PastilleClub nom={space.name} logo={logo} className="h-7 w-7 rounded-lg text-[10.5px]" />
      <span className="min-w-0 flex-1">
