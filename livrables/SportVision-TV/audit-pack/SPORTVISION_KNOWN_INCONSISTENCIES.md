...
- **Systèmes impactés** : `contenus` (calendrier éditorial CM), `contenu_stats` (rapports), `messages_client`, `audit_logs`, RPC `client_valider_contenu`, trigger `contenus_valider_transition_statut`.
- **Ce qui a été testé, avec des données jetables réelles (client Full Communication actif signé, club, membre coach, CM avec portefeuille)** : `brouillon → a_valider_interne → a_valider_client` (staff), demande de corrections côté client (`client_valider_contenu`, décision `corrections`, en tant que membre de club coach — pas le fondateur `client_users`), vérifié que le message client et l'entrée `audit_logs` sont bien écrits avec `auteur_client_id = null` (comportement voulu pour un membre de club, corrigé antérieurement — voir migration-clubplus-v34), reprise `corrections → brouillon`, re-soumission jusqu'à `a_valider_client`, validation client (`valide`), publication (`programme → publie`, `date_publication` posée), saisie de stats CM (`contenu_stats`), lecture des stats côté club (accès rapport), puis `publie → archive`. Testé aussi en négatif : lecture anonyme sans JWT → refusée (permission Postgres, pas de fuite de ligne).
- **Résultat** : les 9 transitions de statut, la RPC de validation client, les policies RLS par étape et la remontée des stats fonctionnent toutes comme documenté — aucun bug trouvé sur le chemin nominal.
- **Point mineur relevé au passage (non corrigé, faible sévérité)** : `client_valider_contenu()` vérifie `v_row.statut = 'a_valider_client'` AVANT de vérifier que l'appelant est autorisé (`client_users`/`club_member_has_client_access`). Un appelant authentifié quelconque (n'importe quel compte Connect, même sans lien avec le client) qui connaît un `contenu_id` peut donc apprendre indirectement si ce contenu est actuellement `a_valider_client` ou non (message d'erreur différent), sans jamais voir son contenu réel. Fuite d'information mineure (un booléen, sur un UUID qu'il faut déjà connaître), pas de PII exposée. À corriger si Fouka le juge utile : inverser l'ordre des deux vérifications dans la RPC.
- **Statut** : **CERTIFIÉ** — pipeline complet vérifié en conditions réelles, résidu de test à zéro.

### INC-033 — Paiement Stripe réussi mais jamais reflété sur la prestation (CRITIQUE, CORRIGÉ)

- **Sévérité** : Critique — bug silencieux affectant potentiellement TOUS les paiements Stripe réels depuis la création du trigger fautif.
- **Découvert en exécutant le dernier P0 restant** : le E2E unique bout-en-bout (Vitrine → OS → production → livrable → facture → paiement) demandé par l'audit — reconstruit intégralement avec des données réelles jetables jusqu'au bout, pour la première fois cette nuit.
- **Comportement** : `stripe-webhook` (edge function, `service_role`) met à jour `prestations.statut_financier` (`payée`/`partiellement_payée`) après un paiement Stripe réussi. Le trigger `protect_sensitive_prestation_fields()` vérifie `auth.uid()`/`profiles.role` pour autoriser cette écriture — mais un appel `service_role` n'a **jamais** de `auth.uid()` (c'est un rôle Postgres, pas un utilisateur authentifié), donc `is_privileged=false` systématiquement, et l'écriture est **rejetée à chaque fois**. Pire : le code du webhook ne vérifie même pas l'erreur retournée (`const { data } = await admin.from("prestations").update(...)`, `error` jamais destructuré) — l'échec est **totalement silencieux**, aucun log, aucune alerte. Conséquence concrète : un client qui paie réellement via Stripe peut voir sa prestation rester affichée `non_facturée`/`en_retard` dans l'OS indéfiniment — risque direct de relancer un client qui a déjà payé.
- **Ampleur du problème, vérifiée** : sur les 16 triggers `protect_sensitive_*_fields` du même patron, **1 seul** (`protect_sensitive_club_fields`) avait déjà un garde `auth.role() = 'service_role'` (ajouté antérieurement pour une raison similaire, jamais propagé). Les **15 autres** — dont `protect_sensitive_prestation_fields` — n'avaient aucun garde de ce type.
- **Reproduit en direct avant correctif** : reconstruit tout le chemin (vraie soumission Vitrine via `create-guest-request`, 18 transitions de statut réelles jusqu'à `production_terminée`, facture générée en répliquant exactement `getOrCreateFacture()`, livrable créé, paiement Stripe simulé) — confirmé que l'appel `service_role` sur `statut_financier` échouait avec `Modification non autorisée...`, exactement le comportement du vrai webhook en production.
- **Correctif** (`migration-securite-v99-protect-sensitive-service-role-bypass.sql`) : ajoute le même garde `if auth.role() = 'service_role' then return new; end if;` aux 15 triggers restants, au même endroit, sans toucher au reste de leur logique — additif pur, patron déjà validé par `protect_sensitive_club_fields`.
- **Vérifié après correctif** : le même appel `service_role` sur `statut_financier` réussit désormais. Un utilisateur authentifié quelconque (sans rôle staff) reste bloqué (RLS filtre la ligne avant même que le trigger s'exécute) — la protection contre un appelant illégitime n'a pas été affaiblie, seul le blocage erroné d'un appelant `service_role` légitime a été levé. Chaîne complète re-testée après correctif : `SV-2026-00XX` → `statut_financier=payée` → facture `payee` → paiement `reussi` → livrable `livre`, tout cohérent. Résidu de test à zéro.
- **Statut** : **CORRIGÉ** (20/08/2026).
...
- **64 fonctions sur 148 n'avaient pas de `search_path` explicite** (les 3 restantes des 67 initiales avaient déjà été corrigées avec leur réécriture ci-dessus) — **corrigé également** (`migration-securite-v98-search-path-hardening.sql`, `ALTER FUNCTION ... SET search_path`, aucune logique modifiée). Vérifié : 0 fonction SECURITY DEFINER restante sans search_path dans le schéma public, spot-check fonctionnel (get_my_role, is_staff) confirmé identique avant/après.
- **Statut** : **INTÉGRALEMENT CORRIGÉ** — les 4 failles actives + le durcissement search_path sur les 148 fonctions. Seul le défaut mineur sur l'XP de formation (`rpc_complete_formation`/`rpc_submit_quiz`, pas de verrou) reste en dette, non urgent (pas de l'argent réel).
...
### INC-029 — Storage : lecture publique sur des chemins conçus comme privés (P0 audit §17, CORRIGÉ)

- **Sévérité** : Haute (structurelle) mais exposition réelle jamais survenue — voir détail.
...
- **Comportement trouvé** : les policies d'ÉCRITURE sur ces deux chemins étaient déjà correctement scopées (client propriétaire/joueur avec accès pour `messages/`, parent confirmé pour `family-docs/`), mais la policy de LECTURE était un simple `bucket_id = X` sans aucune restriction de chemin — accessible à tout le monde, authentifié ou non.
- **Premier correctif (20/08, insuffisant seul)** : `migration-storage-v94-restrict-sensitive-reads.sql` — policies SELECT scopées sur les buckets existants. Testé en direct : **insuffisant**, car `portail-media`/`clubplus-media` ont `bucket.public=true`, et l'endpoint `/object/public/<bucket>/<path>` (celui que génère `getPublicUrl()`, utilisé partout — `<img src>`, liens de pièce jointe) sert le fichier sans jamais consulter la RLS dès que le bucket est public. Passer le bucket en `public=false` corrige la lecture mais casse tout le contenu légitimement public du même bucket (logos, catalogue vitrine, avatars).
- **Correctif définitif (20-21/08)** : `migration-storage-v95-private-bucket-signed-urls.sql` — nouveau bucket **privé** dédié `sportvision-media-prive` + colonne `messages_client.piece_jointe_path` (chemin, pas URL) + policies INSERT/SELECT scopées dessus. Côté code : `MessagesThread.tsx` (Connect, espace joueur) et `MessagesParticulierView.tsx` (Connect, espace particulier) uploadent désormais vers ce bucket privé et génèrent une URL signée (`createSignedUrl`, TTL 1h) à l'affichage au lieu d'une URL publique permanente ; côté OS, l'écran "Messages clients" (qui n'affichait auparavant aucune pièce jointe, texte seul) a reçu un lien "📎 Pièce jointe" qui génère sa propre URL signée à la demande.
- **Exposition réelle constatée** : **nulle**. `family-docs/` n'a aucun code d'upload nulle part dans le repo (policy DB créée par `migration-clubplus-v17.sql`, fonctionnalité jamais construite côté UI — mort-né, pas une fuite active, non traité par ce correctif). `messages_client` était à 0 ligne en production au moment de la découverte — aucune pièce jointe n'avait donc jamais été exposée avant la correction.
- **Statut** : **CORRIGÉ** pour le chemin `messages/` (le seul avec du code d'upload réel). `family-docs/` reste sur l'ancien bucket public mais reste mort-né (aucun risque actif) — à traiter si cette fonctionnalité est un jour construite.
...
### INC-025 — Ledger crédits Club+ : donnée réelle déjà en base, aucun écran ne l'affiche (P0 #14, CORRIGÉ)
...
- **Comportement d'origine** : contrairement à l'hypothèse initiale de l'audit ("le ledger n'existe pas"), la donnée EST déjà réelle et à jour — `club_credit_transactions` est déjà écrite par `update_club_request_status`/`staff_update_club_request_status` (voir INC-024) à chaque consommation de crédit, avec libellé et montant. Mais zéro écran de l'OS ne la lisait — le staff n'avait aucun moyen de voir l'historique des mouvements de crédits d'un club, seulement le solde final s'il allait chercher la ligne `clients` en base directement.
- **Fait le 20/08** : exposé honnêtement dans `/demo/clubplus` (solde par club + mouvements), étiqueté clairement comme donnée réelle sans écran dédié.
- **Correctif définitif (21/08)** : plutôt que d'étendre `connect_os_account_detail` (qui ne couvre que les JOUEURS Connect affiliés à un club, pas le compte organisation Club+ — confusion de modèle documentée en INC-026), le ledger a été branché directement dans la fiche client existante de l'OS (`modalModifierClient`). Quand un client de type club est ouvert, `chargerCreditsProjetClient` détecte la ligne `clubs` liée et appelle la nouvelle fonction `chargerLedgerCreditsClub` : elle affiche le résumé (alloués/mois, solde, réservé, disponible) puis les 10 derniers mouvements de `club_credit_transactions` (date, libellé, montant signé), en lecture seule. Vérifié en E2E réel (client/club/3 mouvements de test créés en base, JWT staff réel, affichage confirmé dans le DOM avec les bons montants et le bon ordre chronologique), données de test supprimées après vérification (résidu confirmé à zéro).
- **Fichiers concernés** : `SportVision-OS-Full.html` (`chargerCreditsProjetClient` et `chargerLedgerCreditsClub`, juste après).
- **Statut** : **CORRIGÉ** — déployé sur main, en cours de mise en ligne Netlify.
