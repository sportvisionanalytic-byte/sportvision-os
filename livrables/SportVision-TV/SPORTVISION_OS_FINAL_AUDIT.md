...
1. **Token GitHub personnel exposé en clair dans `git remote -v`** — signalé depuis le 06/08, toujours non traité 3 semaines plus tard. À révoquer en priorité (GitHub → Settings → Developer settings → Personal access tokens), puis reconfigurer le remote sans token dans l'URL.
2. **4 Edge Functions modifiées cette nuit nécessitent un redéploiement manuel** pour que le correctif de double création client prenne effet en production : `create-guest-request`, `create-guest-rdv`, `portal-onboarding`, `clubplus-onboarding`. Chaque fichier porte l'avertissement d'origine "ne se déploie pas automatiquement depuis le repo" — convention déjà en place dans le projet, respectée cette nuit plutôt que contournée. À coller dans Supabase Dashboard → Edge Functions → Deploy.
3. **Décision produit : pont Club+ "Prestation"** (réservation → Production) — aujourd'hui inexistant, nécessite de définir le mapping de champs et le déclencheur.
4. **Décision produit : séparation des pouvoirs Commercial/Secrétariat** — le rôle `com` a aujourd'hui le même niveau d'écriture que `admin`/`sec` sur `clients`, peut faire passer un prospect en client actif sans validation intermédiaire.
5. **Décision produit : filtre `actif`/formation à l'affectation mission** — un collaborateur en onboarding non finalisé peut aujourd'hui être sélectionné pour une mission réelle.
6. **Plafond de taille/type de fichier des buckets Storage Club+/Portail** — actuellement sans limite, une décision de Fouka sur les formats/tailles légitimes est nécessaire avant d'appliquer une restriction technique.
7. **Stripe live jamais testé avec un vrai cycle de paiement complet** — nécessite une carte réelle ou des clés de test dédiées.
...
- `profiles.actif` non vérifié à l'affectation mission (voir action humaine ci-dessus).
...
| Cohérence | 7/10 |
| Fiabilité | 8/10 |
| Prêt pour usage réel | 7/10 |

L'OS a gagné en fiabilité réelle cette nuit (bugs de date et d'idempotence corrigés et vérifiés par exécution, pas supposés). Ce qui retient encore la note : un token exposé qui reste une vraie faille tant qu'il n'est pas révoqué, un pont métier Club+ entièrement absent, et plusieurs décisions produit encore ouvertes qui conditionnent la suite. Rien de bloquant pour continuer à faire vivre l'OS au quotidien ; les points de la section 10 méritent une vraie priorisation à ton réveil.
