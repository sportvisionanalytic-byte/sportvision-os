...
do $garde$
begin
  -- GARDE-FOU ADAPTE A REVIEW (12/09/2026). L'original compare 50 empreintes md5 a l'inventaire
  -- de PRODUCTION du 10/09. Mesure faite sur Review le 12/09 : 45 empreintes sur 50 identiques,
  -- 5 ecarts, tous expliques et tous sans danger a ecraser ici :
  --   * get_my_role()                          — derive Review connue (sans « and actif »), c'est
  --                                               precisement ce que cette migration corrige ;
  --   * modifier_remuneration_mission(...)     — Review porte deja la version v148, plus recente ;
  --   * pole_valider_remuneration_responsable  — Review porte deja la version v150, plus recente ;
  --   * staff_update_club_request_status(...)  — derive Review ;
  --   * rpc_retirer_membre_equipe(uuid)        — ABSENTE de Review : elle n'est creee par aucune
  --                                               migration du depot (objet ne en production hors
  --                                               migration) ; cette migration la cree.
  -- Les deux fonctions marquees « plus recente » sont ramenees a la version du 10/09 par cette
  -- migration, puis v148 et v150 sont rejouees juste apres pour les remettre a l'etat final.
  if exists (select 1 from pg_proc where proname = 'compte_os_desactive'
               and pronamespace = 'public'::regnamespace) then
    raise exception 'migration-decisions-os-v1 deja appliquee : rien n''a ete modifie.';
  end if;
end $garde$;
