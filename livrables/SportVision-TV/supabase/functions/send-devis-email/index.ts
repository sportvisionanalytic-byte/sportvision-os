...
// send-devis-email → coller ce code → Deploy.
...
// Supabase Edge Function — send-devis-email
// Fetches devis data from Supabase and sends a professional HTML email via Resend.
// Deploy via Supabase dashboard > Edge Functions > New Function (name: send-devis-email)
...
serve(async (req) => {
  ...
  try {
    ...
    if (!devis_id || !to) {
      return new Response(JSON.stringify({ error: "devis_id et to sont requis" }), {
        status: 400,
        headers: { ...corsHeaders, "Content-Type": "application/json" },
      });
    }
    ...
    // n'importe quel devis a l'adresse de son choix. Meme regle que send-facture-pennylane.
    ...
    const d = devisArr[0];
    const c = d.clients || {};
    const lignes: Array<{ description?: string; libelle?: string; details?: string; quantite?: number; qte?: number; prix_unitaire?: number; pu?: number }> = Array.isArray(d.lignes) ? d.lignes : [];
    ...
    const stLbl: Record<string, string> = {
      brouillon: "Brouillon", envoyé: "Envoyé", en_attente: "En attente",
      accepté: "Accepté", refusé: "Refusé", expiré: "Expiré",
    };
    const stClr: Record<string, string> = {
      brouillon: "#8891A8", envoyé: "#366BFF", accepté: "#2ECC8A",
      refusé: "#F0445E", expiré: "#F5A524",
    };
    ...
    const lignesHtml = lignes.length
      ...
      : `<tr><td colspan="4" style="padding:16px;color:#555E73;font-size:12.5px">
          <div style="font-weight:600">${d.prestation?.description_besoin || "Prestation SportVision"}</div>
          ${d.prestation?.livrables_demandes ? `<div style="margin-top:4px;color:#8891A8">${d.prestation.livrables_demandes}</div>` : ""}
        </td></tr>`;

    const remiseHtml =
      (d.remise_montant || 0) > 0
        ? `<tr><td style="padding:5px 0;font-size:12.5px;color:#555E73;border-bottom:1px solid #E8EBF4">Sous-total HT</td><td style="padding:5px 0;text-align:right;font-size:12.5px;color:#555E73;border-bottom:1px solid #E8EBF4;font-variant-numeric:tabular-nums">${fmt(d.sous_total || 0)}</td></tr>
           <tr><td style="padding:5px 0;font-size:12.5px;color:#F5A524;border-bottom:1px solid #E8EBF4">Remise (${d.remise_pct || 0}%)</td><td style="padding:5px 0;text-align:right;font-size:12.5px;color:#F5A524;border-bottom:1px solid #E8EBF4;font-variant-numeric:tabular-nums">- ${fmt(d.remise_montant)}</td></tr>`
        : "";

    // Message lines as HTML paragraphs
...
<title>${subject || "Devis — SportVision"}</title></head>
  ...
  <div style="max-width:680px;margin:32px auto;background:#fff;border-radius:14px;overflow:hidden;box-shadow:0 4px 24px rgba(0,0,0,.08)">
    ...
    <div style="background:#1a1a2e;padding:28px 36px;display:flex;justify-content:space-between;align-items:center">
      ...
      <div style="text-align:right">
        <div style="font-size:26px;font-weight:800;color:#366BFF;letter-spacing:-.01em">DEVIS</div>
        <div style="font-size:12px;color:#8891A8;margin-top:2px">${d.numero || "—"}</div>
        <div style="margin-top:6px"><span style="display:inline-block;padding:2px 10px;border-radius:20px;font-size:9.5px;font-weight:700;text-transform:uppercase;letter-spacing:.08em;background:${stClr[d.statut] || "#8891A8"}33;color:${stClr[d.statut] || "#8891A8"}">${stLbl[d.statut] || d.statut}</span></div>
      </div>
    </div>
    ...
    <div style="margin:24px 36px;background:#F5F7FF;border-radius:10px;padding:16px 20px;display:grid;grid-template-columns:repeat(3,1fr);gap:16px">
      <div><div style="font-size:9px;font-weight:700;text-transform:uppercase;letter-spacing:.1em;color:#8891A8;margin-bottom:4px">Émission</div><div style="font-size:12.5px;font-weight:600;color:#1a1a2e">${fd(d.created_at)}</div></div>
      <div><div style="font-size:9px;font-weight:700;text-transform:uppercase;letter-spacing:.1em;color:#8891A8;margin-bottom:4px">Validité</div><div style="font-size:12.5px;font-weight:600;color:#1a1a2e">${d.validite_jours || 30} jours</div></div>
    ...
    <div style="margin:0 36px 24px">
      ...
      <div style="display:flex;justify-content:flex-end;margin-top:4px">
        <table style="width:280px;border-collapse:collapse">
          ${remiseHtml}
          <tr><td style="padding:6px 0;font-size:12.5px;color:#555E73;border-bottom:1px solid #E8EBF4">Total HT</td><td style="padding:6px 0;text-align:right;font-size:12.5px;color:#555E73;border-bottom:1px solid #E8EBF4;font-variant-numeric:tabular-nums">${fmt(d.total_ht || 0)}</td></tr>
          <tr><td style="padding:6px 0;font-size:12.5px;color:#555E73;border-bottom:1px solid #E8EBF4">TVA (${d.tva_pct || 20}%)</td><td style="padding:6px 0;text-align:right;font-size:12.5px;color:#555E73;border-bottom:1px solid #E8EBF4;font-variant-numeric:tabular-nums">${fmt((d.total_ttc || 0) - (d.total_ht || 0))}</td></tr>
          <tr><td style="padding:12px 0;font-size:17px;font-weight:800;color:#1a1a2e;border-top:2px solid #1a1a2e">TOTAL TTC</td><td style="padding:12px 0;text-align:right;font-size:17px;font-weight:800;color:#366BFF;border-top:2px solid #1a1a2e;font-variant-numeric:tabular-nums">${fmt(d.total_ttc || 0)}</td></tr>
        </table>
      </div>
    </div>
    ...
    const resendRes = await fetch("https://api.resend.com/emails", {
      ...
      body: JSON.stringify({
        from: fromEmail,
        to: [to],
        subject: subject || `Devis ${d.numero || ""} — SportVision`,
        html: htmlEmail,
      }),
    });
