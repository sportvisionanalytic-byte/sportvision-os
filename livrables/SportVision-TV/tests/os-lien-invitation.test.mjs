...
// Deux personnes reelles : celle qui recoit l'invitation, et celle dont la session traine deja
// dans le navigateur. Choisies dans la base pour que le test mesure de vraies policies.
const r0 = await fetch(`${SB}/rest/v1/profiles?select=id,prenom,role&role=eq.photo&actif=is.true&order=created_at.asc&limit=2`,
  { headers: { apikey: KEY, Authorization: `Bearer ${KEY}` } });
const gens = await r0.json();
if (!Array.isArray(gens) || gens.length < 2) {
  console.log("Il faut deux photographes actifs en base — test ignore.");
  process.exit(0);
}
const [invite, occupant] = gens;
