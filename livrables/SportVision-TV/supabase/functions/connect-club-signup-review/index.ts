...
// Appelée par le STAFF SportVision depuis le nouvel écran de traitement des
// demandes de SportVision OS pour traiter une ligne connect_club_signup_requests
// (migration-connect-v44-club-signup-requests.sql). Trois actions :
//
//  - "valider" : le staff CHOISIT EXPLICITEMENT le rôle Connect initial du
//    contact (initial_role — jamais déduit de la fonction déclarée dans le
//    club, cf. en-tête de la migration v44) et génère un lien d'activation
//    privé, sur le même patron exact que clubplus-generate-activation :
//    résout ou crée la fiche `clients` CRM (le prospect n'en a par
//    définition aucune avant sa demande publique), crée une ligne
//    clubplus_activation_tokens (client_id, initial_role, source_request_id),
//    et NE CRÉE NI club NI club_members ici — ça reste le travail de
//    clubplus-activate, à la consommation du token par le contact. Le lien
//    n'est jamais envoyé automatiquement, c'est le staff qui le copie et le
//    transmet lui-même (règle constante de ce projet).
//  - "demander_infos" : change juste le statut + note, aucune écriture
//    ailleurs.
//  - "refuser" : change juste le statut + note, aucune écriture ailleurs.
//
...
// Secret optionnel : CONNECT_URL (même variable que clubplus-generate-activation)
...
// clubplus_activation_tokens.initial_role (migration-connect-v44).
const VALID_ROLES = [
...
serve(async (req) => {
  ...
  try {
    ...
    if (!callerProfile || !STAFF_ROLES.includes(callerProfile.role)) {
      return json({ error: "Seul le staff SportVision peut traiter une demande d'ouverture Connect." }, 403);
    }
    ...
    const { data: reqRow } = await admin
      ...
      .select("id, club_nom, ville, contact_prenom, contact_nom, contact_email, contact_telephone, statut")
      .eq("id", requestId)
      .maybeSingle();
    ...
    const clubNomOverride: string = (body.club_nom_override || "").trim();
    ...
    if (matchedClient) {
      ...
      const { data: createdClient, error: clientErr } = await admin
        ...
        .insert({
          statut: "prospect",
          type_client: "club",
          nom: clubNomOverride || reqRow.club_nom,
    ...
    const clubNomPrefill = clubNomOverride || reqRow.club_nom;
    ...
    const { data: createdToken, error: tokErr } = await admin
      ...
      .insert({
        client_id: clientId,
        token,
        club_nom_prefill: clubNomPrefill,
        plan,
