...
- **Rémunération/paie** : `employee_costs` (salaire brut, charges patronales) est réservée admin/compta par un contrôle JS explicite (`canSeePaie`). Sur `prestations_equipe.remuneration` : **corrigé le 20/08** après un test RBAC en conditions réelles (comptes jetables) ayant confirmé que `prod` pouvait lire `remuneration` via un appel API direct malgré le masquage côté UI. Migration `migration-prestations-equipe-v88` : la table brute n'autorise plus `prod` en SELECT (seuls admin/sec/self), et une vue `prestations_equipe_display` (utilisée par les 38 lectures frontend) masque `remuneration` à NULL pour tout rôle hors admin/sec/compta/expert_comptable/auditeur/self, tout en gardant les autres colonnes visibles pour que `prod` continue à gérer l'équipe normalement. `sec` garde volontairement l'accès complet (y compris `remuneration`) — c'est la répartition métier demandée explicitement (secrétariat + admin posent la rémunération, prod affecte l'équipe et ajoute des consignes sans la voir).
